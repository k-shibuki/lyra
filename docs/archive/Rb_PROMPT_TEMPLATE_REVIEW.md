...
>
> **Archived**: 2025-12-27
> **Updated**: 2025-12-31 (追補8: 3Bモデル向けプロンプト最適化)
...
### 8) 3Bモデル向けプロンプト最適化（2025-12-31）

**背景:**
- ローカルLLM (qwen2.5:3b, 8GB VRAM) で長いプロンプトが空の出力を返す問題
- セキュリティタグ付きプロンプト (~1700字) でクレーム抽出が失敗

**対応:**

1. **セキュリティタグのデフォルト無効化**
   - `src/utils/config.py`: `session_tags_enabled` デフォルトを `False` に変更
   - 理由: 3Bモデルは長いプロンプトの処理が苦手
   - 7B以上のモデルでは `.env` で有効化可能
...
| 問題 | 対策 |
|------|------|
| 具体的な値 (0.8, 0.9) をコピーする | `<0.0-1.0>` 形式のプレースホルダーを使用 |
| 抽出件数が不安定 | 「1-5件」「3-10件」と明示的に制限 |
| 重要な情報の欠落 | 「数値・日付・固有名詞を優先」と明示 |
| 形式の不一致 | JSON Schema (`format` パラメータ) で強制 |

4. **JSON Schema による出力形式強制**
   - `src/filter/llm.py`: `EXTRACT_CLAIMS_SCHEMA`, `EXTRACT_FACTS_SCHEMA` を定義
   - Ollama の `format` パラメータに JSON Schema を渡すことで配列形式を強制
   - プロンプトは意味的な指示に集中し、形式はスキーマで保証

**テンプレート変更概要:**

| ファイル | 変更前 | 変更後 |
|----------|--------|--------|
| `extract_claims.j2` | 詳細なスキーマ説明 | 1行の指示 + プレースホルダー |
| `extract_facts.j2` | 詳細なルール | 1行の指示 + 件数制限 |
| `decompose.j2` | 長いExample | 簡潔なスキーマ |
| `densify.j2` | 詳細な構造説明 | 1行の指示 |
| `initial_summary.j2` | 詳細な構造説明 | 1行の指示 |
| `quality_assessment.j2` | 詳細なガイドライン | 2行の基準 |
| `detect_citation.j2` | 詳細なルール | 1行の判定基準 |
| `relevance_evaluation.j2` | 詳細なスケール | 1行のスケール |
| `summarize.j2` | 詳細な要件 | 1行の指示 |
| `translate.j2` | 詳細なルール | 1行の指示 |

**検証結果:**
- 3Bモデルで安定してクレーム抽出が成功 (5件/回)
- 値の固定化問題が解消 (relevance: 0.8, 0.85 など動的に変化)
- 全86テストが通過
