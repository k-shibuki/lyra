...
4. **依存関係不足時のテストスキップ**: 必要なパッケージがインストールされていない場合、該当テストファイルは収集されません

### ⚠️ 重要: ローカルでの追加テスト実行が必要

**クラウドエージェント環境で実行されたテストは、プロジェクト全体のテストカバレッジを満たしていません。**

以下のテストは**必ずローカル環境で追加実行**してください：

```bash
# E2Eテスト（Chrome CDP、Ollama、ネットワーク接続が必要）
pytest -m e2e
...
クラウドエージェントでテストを実行すると、以下のような通知が表示されます：
...
======================================================================
  ...
  ⚠️  E2E/Slow tests are SKIPPED in this environment.
  📋 Please run the following tests LOCALLY:
