...
- **よくある事故パターン（回収ポイント）**:
  - **「新規パラメータの値は検証しているが、どこにも伝播しておらず実際は機能しない」**:
    - `/task-plan` の計画に **伝播マップ**（どこで受け取り、どこに渡し、どこで効果が出るか）を含める
    - 複数モジュール/境界を跨ぐ場合は `/integration-design` を使い、**シーケンス図・契約（Pydantic）・デバッグスクリプト**で境界ごとに伝播を検証する
    - `/test-create` では **wiring/effect テスト**（下流呼び出し引数や生成リクエスト/クエリの検証、値変更による挙動変化の検証）を少なくとも1つ入れて、未配線をテストで検出する
    - `/test-review` では「値検証のみで、伝播/効果の検証がない」テストを差し戻す
