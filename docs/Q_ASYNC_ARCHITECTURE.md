...
### Phase 3b: 用語・パラメータ衝突のクリーンアップ（Breaking / 互換なし）✅ DONE
...
- SERP pagination: `serp_page` / `serp_max_pages`（旧 `SearchOptions.page` は削除済み）

**完了条件:**
- [x] `src/`, `config/`, `docs/`, `tests/` から旧用語が消える（grepで残骸ゼロ）
...
- [x] ADR-0013 承認・実装完了
- [x] ADR-0014 承認・実装完了
- [x] グローバルレートリミッター実装（学術API）
- [x] TabPool(max_tabs=1) 実装（ブラウザSERP: Page競合排除）
- [x] テスト追加
- [x] 全テストパス
