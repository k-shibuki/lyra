...
**§2.1.5判断委譲の例外**:
- Cursor AI無応答時（60秒既定）: パイプライン安全停止、状態保存、復帰待機
  - **実装状況**: ✅ **実装済み**。パイプラインタイムアウト（60秒）とアクティビティ追跡を実装。`get_status`で`idle_seconds`と警告を返却。
...
- [x] README.md（プロジェクト概要、セットアップ手順） - ✅ **作成完了**（2025-12-19）
- [ ] 手動介入運用手順書（§6成果物）
- [ ] MCPツール仕様書（§6成果物）
- [ ] ジョブスケジューラ仕様書（§6成果物）
- [ ] プロファイル健全性監査運用手順
...
**詳細ドキュメント/実装ロードマップ**: `docs/P_EVIDENCE_SYSTEM.md`
...
**詳細ドキュメント/実装ロードマップ**: `docs/Q_ASYNC_ARCHITECTURE.md`

**状態**: 未着手

### Phase R: NLI LoRAファインチューニング ⏳

**目的**: 蓄積されたNLI訂正サンプルを使用して、NLIモデル（DeBERTa-v3）をLoRA（Low-Rank Adaptation）でファインチューニングし、ドメイン固有の精度を向上させる。

**詳細ドキュメント/実装ロードマップ**: `docs/R_LORA_FINETUNING.md`
