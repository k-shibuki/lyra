...
- Cursor AI無応答時（300秒既定）: パイプライン安全停止、状態保存、復帰待機
  - **実装状況**: ⏳ **未実装**。MCPプロトコル上、Cursor AIからの呼び出し待機は非同期で行われるため、300秒タイムアウト検知は技術的に困難。現状は`wait_for_user`ツールのタイムアウト（300秒）のみ実装済み。パイプライン安全停止機能は未実装（Phase M.6参照）。
...
**目的**: LLMプロンプトテンプレートをPythonコードから外部ファイル（Jinja2）に分離し、保守性と柔軟性を向上。

**実装内容**:

| 項目 | ファイル | 状態 |
|------|---------|:----:|
| 依存関係 | `requirements-mcp.txt` (jinja2>=3.1.0追加) | ✅ |
| PromptManager | `src/utils/prompt_manager.py` | ✅ |
| extract_facts | `config/prompts/extract_facts.j2` | ✅ |
| extract_claims | `config/prompts/extract_claims.j2` | ✅ |
| summarize | `config/prompts/summarize.j2` | ✅ |
| translate | `config/prompts/translate.j2` | ✅ |
| decompose | `config/prompts/decompose.j2` | ✅ |
...
prompt = render_prompt("extract_claims", text="...", context="リサーチクエスチョン")
```

**注意**: L4検出用テンプレート（`EXTRACT_FACTS_INSTRUCTION`等）は`TASK_INSTRUCTIONS`辞書で管理されており、外部化対象外。
...
- [ ] README.md（プロジェクト概要、セットアップ手順） - **現状**: `README.md`ファイルは存在するが、内容は「完成後に作成（WIP）」のみ。実質的に未作成。
- [ ] 手動介入運用手順書（§6成果物）
- [ ] MCPツール仕様書（§6成果物）
- [ ] ジョブスケジューラ仕様書（§6成果物）
- [ ] プロファイル健全性監査運用手順
...
| 項目 | 状態 | 備考 |
|------|:----:|------|
| Cursor AI無応答ハンドリング（§2.1.5） | ⏳ | **未実装**: MCPプロトコル上、Cursor AIからの呼び出し待機は非同期で行われるため、300秒タイムアウト検知は困難。現状は`wait_for_user`ツールのタイムアウト（300秒）のみ実装済み。パイプライン安全停止機能は未実装。 |
