...
Lyraは**コンテキストエンジニアリングの一部**であり、すべてを自動処理することが目的ではない。

| 役割 | 自動化（Lyra） | 人間/Cursor AI |
|------|:----------------:|:--------------:|
| **論文発見** | ✅ | - |
| **メタデータ取得** | ✅ | - |
| **抄録によるエビデンス** | ✅ | - |
...
| 2025-12-16 | **Abstract Only戦略採用**: PDFフルテキスト取得を設計上排除。学術論文は抄録＋メタデータのみ自動取得し、フルテキストは参照先（DOI/OA URL）として提示。Lyraは「コンテキストエンジニアリングの一部」として信頼性の高いサジェストに専念する設計思想を明確化 |
