...
show_help() {
    ...
    echo ""
    echo "What doctor checks (13 items):"
    echo "  - Environment: WSL2/Linux detection"
    echo "  - Make: make command (required for Makefile workflow)"
    echo "  - Python/uv: uv command, .venv existence, Python 3.14+ version"
    echo "  - Rust: rustc version (>= 1.82) and rustup availability"
    echo "  - Container: podman/docker and compose availability"
    echo "  - GPU: nvidia-smi for GPU presence"
    echo "  - GPU (Container): Podman CDI or Docker nvidia-container-toolkit"
    echo "  - Disk space: ~25GB required for ML images and models"
    echo "  - Chrome: Browser installation (snap chromium or Google Chrome)"
