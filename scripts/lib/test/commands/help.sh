...
show_help() {
    ...
    echo "  --auto        Prefer venv by default (container only when explicitly forced, or when already running inside it)"
    ...
    echo "  LYRA_TEST_SHOW_TAIL_ON_SUCCESS=true  Show tail even when tests pass"
    echo "  LYRA_TEST_JSON_DETAIL=full|minimal   Control JSON artifact verbosity (default: full)"
    ...
    echo ""
    echo "Note: Tests run in venv (.venv) by default. Use --container or RUNTIME=container to run inside containers."
    echo ""
