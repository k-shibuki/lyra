...
- Block-resistant (priority 2 in engines.yaml)
- UK-based, privacy-focused
- No tracking, no profiling

Verification items:
1. CDP connection
2. Mojeek search operation
3. Search result parser accuracy (MojeekParser)
4. CAPTCHA detection
...
Usage:
    podman exec lancet python tests/scripts/verify_mojeek_search.py
...
Note:
    This script accesses real Mojeek search.
    Mojeek is block-resistant but still run in low-risk IP environment.
...
class MojeekSearchVerifier:
    """Verifier for Mojeek search functionality."""
    
    ENGINE_NAME = "mojeek"
    ENGINE_DISPLAY = "Mojeek"
    ...
    async def check_prerequisites(self) -> bool:
        ...
        print("    Mojeek is block-resistant, but use caution.\n")
        return True
    ...
    async def verify_search(self) -> VerificationResult:
        """Verify Mojeek search operation."""
    ...
    async def verify_captcha_detection(self) -> VerificationResult:
        ...
        try:
            ...
            test_cases = [
                ("<html><body>unusual traffic detected</body></html>", True, "rate_limit"),
                ("<html><body>verify you're not a robot</body></html>", True, "captcha"),
