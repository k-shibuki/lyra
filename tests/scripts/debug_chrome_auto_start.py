...
async def main():
    ...
    try:
        ...
        print(f"  ✓ _ensure_browser() returned")
        print(f"    browser: {browser is not None}")
        print(f"    context: {context is not None}")
        
        if browser:
            print(f"  ✓ Browser obtained successfully")
        else:
            print(f"  ✗ Browser is None")
            
    except asyncio.TimeoutError:
        print(f"  ⚠ Timeout: _ensure_browser() took longer than 45 seconds")
