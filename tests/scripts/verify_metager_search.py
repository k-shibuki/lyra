#!/usr/bin/env python3
"""
MetaGer Search Engine E2E Verification

Verification target: §3.2 Agent Execution (Browser Search) - MetaGer

MetaGer characteristics:
- German meta-search engine
- High block resistance (privacy-focused)
- Aggregates results from multiple sources
- Supports German and English queries

Verification items:
1. CDP connection
2. MetaGer search operation
3. Search result parser accuracy (MetaGerParser)
4. CAPTCHA detection (German patterns)
...
Usage:
    podman exec lancet python tests/scripts/verify_metager_search.py
...
Note:
    This script accesses real MetaGer search.
    MetaGer has high block resistance but still use caution.
...
class MetaGerSearchVerifier:
    """Verifier for MetaGer search functionality."""
    
    ENGINE_NAME = "metager"
    ENGINE_DISPLAY = "MetaGer"
    ...
    async def check_prerequisites(self) -> bool:
        ...
        print("    MetaGer has high block resistance but use caution.\n")
        return True
    ...
    async def verify_search(self) -> VerificationResult:
        """Verify MetaGer search operation."""
        ...
        try:
            # Use English query (MetaGer supports both German and English)
            test_query = "open source software development"
    ...
    async def verify_parser_accuracy(self) -> VerificationResult:
        ...
        try:
            ...
            test_query = "GitHub code repository"
            ...
            # Check for github in results
            has_expected = any("github" in r.url.lower() for r in result.results)
            if has_expected:
                print("    ✓ Expected result (github) found")
            else:
                print("    ! Expected result (github) not found")
...
async def main():
    ...
    verifier = MetaGerSearchVerifier()
