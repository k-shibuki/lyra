...
async def test_last_search_times_tracking():
    ...
    print(f"  ✓ Provider has '_last_search_times' attribute")
...
async def test_rate_limit_engine_parameter():
    ...
    try:
        ...
        if "unexpected keyword argument" in str(e) or "positional argument" in str(e):
            print(f"    ✗ _rate_limit() doesn't accept 'engine' parameter")
            print(f"    Error: {e}")
            return False
        raise
...
async def test_unknown_engine_fallback():
    ...
    try:
        ...
        print(f"    Uses default interval as fallback")
        return True
    except Exception as e:
...
async def test_full_qps_flow():
    ...
    if hasattr(provider, '_last_search_times'):
        ...
        print(f"    ✓ Rate limiting flow completed")
        return True
    else:
        print(f"    ✗ _last_search_times not found")
        return False
