...
Verification items:
1. CDP connection
2. Google search operation
3. Search result parser accuracy (GoogleParser)
4. CAPTCHA detection
...
WARNING:
    Google has VERY aggressive bot detection!
    - CAPTCHA is expected and considered success
    - Only run in low-risk IP environment
    - Prefer other engines (Ecosia/Startpage for Google results)

Related scripts:
    - verify_duckduckgo_search.py
    - verify_startpage_search.py (Google proxy, safer)
"""
...
class GoogleSearchVerifier:
    """Verifier for Google search functionality."""
    ...
    async def check_prerequisites(self) -> bool:
        ...
        print(f"  ⚠ WARNING: {self.ENGINE_DISPLAY} has VERY aggressive bot detection!")
        print("    - CAPTCHA is expected and counts as successful detection")
        print("    - Daily limit: 10 queries")
        print("    - Prefer Startpage/Ecosia for Google results\n")
        return True
    ...
    async def verify_search(self) -> VerificationResult:
        """Verify Google search operation."""
                ...
                if _is_captcha_error(result):
                    ...
                    return VerificationResult(
                        ...
                        details={
                            ...
                            "note": "CAPTCHA expected for Google - detection working",
                        },
                    )
                else:
                    return VerificationResult(
    ...
    async def run_all(self) -> int:
        ...
        if failed > 0:
            ...
            print("  Note: CAPTCHA detection counts as success for Google.")
            return 0
