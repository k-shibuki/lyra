...
- 入力:
  - `action`: "add_sample" | "get_stats" | "rollback" | "evaluate" | "get_evaluations" | "get_diagram_data"
  - `data`: アクションに応じたデータ
    - `add_sample`: `{source, prediction, actual, context?}`
    - `rollback`: `{source, version?, reason?}`
...
- **対応判断**: Cursor AIが`calibrate(action: "rollback")`を呼ぶかどうかを決定（Lancetは自動ロールバックを提案しない）
