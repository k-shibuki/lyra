...
**Phase 16.9.1: プロトタイプ検証（1日）**
- [ ] DuckDuckGo 1エンジンでプロトタイプ実装
- [ ] Playwright経由の検索動作確認
- [ ] CAPTCHA発生→手動解決→検索継続のフロー検証
- [ ] セッション転送が効くことを確認

**Phase 16.9.2: BrowserSearchProvider実装（2-3日）**
- [ ] `BrowserSearchProvider` クラス
...
**Phase 16.9.3: 検索結果パーサー実装（2-3日）**
- [ ] `SearchResultParser` 基底クラス
- [ ] `DuckDuckGoParser` - DDG結果パース
- [ ] `GoogleParser` - Google結果パース（オプション）
- [ ] `MojeekParser` - mojeek結果パース
- [ ] パーサーレジストリ（エンジン→パーサー対応）

**Phase 16.9.4: 統合・移行（1-2日）**
- [ ] `SearchProviderRegistry` にBrowserSearchProvider登録
- [ ] デフォルトプロバイダをBrowserSearchProviderに変更
- [ ] `searxng.py`, `searxng_provider.py` を廃止予定マーク
- [ ] 既存テストの修正
...
**ユニットテスト（モック使用）**:
- [ ] `test_browser_search_provider.py`
  - `test_search_basic`: 基本検索
  - `test_captcha_detection`: CAPTCHA検知
  - `test_intervention_queue_integration`: 介入キュー連携
  - `test_session_reuse`: セッション再利用
- [ ] `test_search_parsers.py`
  - `test_duckduckgo_parser`: DDGパース
  - `test_mojeek_parser`: mojeekパース
  - `test_empty_results`: 結果なしケース
