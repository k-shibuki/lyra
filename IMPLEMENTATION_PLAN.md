...
- **IPリスク**: 開発環境（クラウド/VPS）のIPは検索エンジンからブロックされやすい
- **抗堪性要件との矛盾**: Lancetの§4.3抗堪性設計がテスト時に無効化される
...
| `@e2e @external` | 中 | SearXNG（mojeek等） | ❌ 除外 |
| `@e2e @rate_limited` | 高 | DuckDuckGo/Brave | ❌ 除外 |
...
- [ ] **CAPTCHA検知→誘導フロー**
  - SearXNG `unresponsive_engines`検知
  - InterventionQueueへの自動追加
  - 手動解決後の再試行フロー
- [ ] **モック化戦略**（CI安定化用）
  - VCR/レコード再生方式の導入
...
→ **Phase 14.3.2に統合**。E2E設計・リスク管理はテスト実装フェーズで一元管理する。
