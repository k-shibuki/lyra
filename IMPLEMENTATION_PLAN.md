...
- `src/mcp/schemas/`: 新スキーマ定義（Phase K.3-9で実装予定）
...
| `extract_content` 内部化 | `src/extractor/` 直接呼び出しへ | ✅ |
...
- `src/research/materials.py`: レポート素材取得 ✅
...
論文投稿に必要な「動くソフトウェア」の実証。統合動作確認とケーススタディの実施・記録。

### N.1 目的

| 観点 | 目的 |
|------|------|
| 統合動作確認 | Phase A-Hの成果が連結して動作することの実証 |
...
| ID | シナリオ | 対象 | 検証ポイント |
|----|---------|------|-------------|
| CS-1 | 医薬品安全性情報収集 | FDA/PMDA/EMA横断 | 複数ソース統合、エビデンスグラフ構築 |
| CS-2 | 規制変更追跡（任意） | 添付文書変更履歴 | 時系列情報収集、Wayback連携 |

**シナリオ詳細（CS-1）**:
- 対象: リラグルチド（Liraglutide）の安全性情報
- クエリ例: "liraglutide FDA safety alert", "リラグルチド PMDA 安全性速報"
- 期待成果: FDA/PMDA/EMAからの安全性情報を収集し、エビデンスグラフで関係性を可視化
...
1. 環境準備
   - ./scripts/dev.sh up
   - Chrome リモートデバッグモード起動
   - Ollama モデルロード確認

2. タスク作成
   - Cursor AI: create_task("リラグルチドの安全性情報調査")

3. 検索クエリ設計・実行（Cursor AI主導）
