...
| スクリプト | 検証内容 | 仕様 | 状態 |
|-----------|----------|------|------|
| `verify_duckduckgo_search.py` | DuckDuckGo検索、パーサー精度、セッション管理 | §3.2 | ✅ E2E成功 |
| `verify_ecosia_search.py` | Ecosia検索（Bing再販）、パーサー精度 | §3.2 | 🔄 セレクター修正要 |
| ~~`verify_metager_search.py`~~ | ~~MetaGer検索（ドイツ、高耐性）~~ | §3.2 | ❌ 削除予定 |
| `verify_startpage_search.py` | Startpage検索（Google再販）、パーサー精度 | §3.2 | 🔄 セレクター修正要 |
...
┌─────────────────────────────────────────────────────────────────────┐
│ Cursor AI                                                          │
│   └─ MCPツール呼び出し（search_serp等）                              │
└───────────────────────────────┬─────────────────────────────────────┘
                                │ MCP Protocol
                                ▼
┌─────────────────────────────────────────────────────────────────────┐
│ Lancet MCP Server (src/mcp/server.py)                              │
│   └─ search_serp → BrowserSearchProvider.search()                   │
└───────────────────────────────┬─────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────┐
│ BrowserSearchProvider (src/search/browser_search_provider.py)       │
│   ├─ Playwright CDP接続（Windows Chrome）                           │
│   ├─ 検索URL構築・実行                                              │
│   ├─ CAPTCHA検知 → InterventionManager                              │
│   └─ 結果パース → SearchParsers                                     │
└───────────────────────────────┬─────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────┐
│ SearchParsers (src/search/search_parsers.py)                        │
│   ├─ DuckDuckGoParser ✅                                            │
│   ├─ MojeekParser ✅                                                │
│   ├─ GoogleParser ✅                                                │
│   ├─ BraveParser ✅                                                 │
...
| エンジン | 仕様書§3.2 | engines.yaml | パーサー実装 | E2Eスクリプト | E2E検証 | 状態 |
|----------|:----------:|:------------:|:------------:|:-------------:|:-------:|------|
| DuckDuckGo | ✅ | ✅ | ✅ | ✅ | ✅ 5/5 | **完了** |
| Mojeek | ✅ | ✅ | ✅ | ❌ | - | パーサー済 |
| Google | ✅ | ✅ | ✅ | ❌ | - | パーサー済 |
...
**凡例**:
- 仕様書§3.2: requirements.md に対応エンジンとして記載
- engines.yaml: config/engines.yaml に定義あり
- パーサー実装: search_parsers.py にクラス実装あり
- E2Eスクリプト: tests/scripts/verify_*_search.py 存在
- E2E検証: 実際に実行して成功
...
| スクリプト | 対象 | 状態 | 備考 |
|-----------|------|:----:|------|
| `verify_duckduckgo_search.py` | §3.2 | ✅ | 5/5パス |
| `verify_ecosia_search.py` | §3.2 | ✅ | 4/4パス |
| `verify_startpage_search.py` | §3.2 | ✅ | 4/4パス |
| `verify_mojeek_search.py` | §3.2 | ❌ | 未作成 |
| `verify_qwant_search.py` | §3.2 | ❌ | 未作成 |
| `verify_google_search.py` | §3.2 | ❌ | 未作成（CAPTCHA高リスク） |
| `verify_brave_search.py` | §3.2 | ❌ | 未作成 |
| `verify_bing_search.py` | §3.2 | ❌ | 未作成（パーサーも未実装） |
| `verify_captcha_flow.py` | §3.6.1 | ✅ | 認証キューフロー検証 |
| `verify_session_transfer.py` | §3.1.2 | ✅ | セッション移送検証 |
| `verify_search_then_fetch.py` | §3.2 | ✅ | 検索→取得一貫性 |
| `verify_network_resilience.py` | §7 | ✅ | IPv6/DNS/304検証 |
| `verify_profile_health.py` | §7 | ✅ | プロファイル健全性 |

##### MCPサーバ動作状況

- **実装**: `src/mcp/server.py` ✅
- **動作検証**: ❓ 未確認（E2EスクリプトはMCP経由ではなくPythonモジュール直接使用）
- **認証キュー連携**: ✅ 実装済み（InterventionManager, InterventionQueue）
...
| InterventionManager | ✅ | ✅ |
| InterventionQueue | ✅ | ✅ |
| MCPツール（get_pending_authentications等） | ✅ | ❓ |
| verify_captcha_flow.py | ✅ | ✅ |

---

#### 16.13.1 追加エンジン（Phase 16.13で追加） ✅
- [x] `ecosia`: Bing再販、比較的緩い → **E2E成功**
- [x] `startpage`: Google再販、プライバシー重視 → **E2E成功**
- [x] ~~`metager`~~: **削除**（ログインゲートのため使用不可）

#### 16.13.2 パーサー実装 ✅
- [x] 7エンジンのパーサー実装済み
  - DuckDuckGo, Mojeek, Google, Brave, Qwant, Ecosia, Startpage
- [ ] **Bing未実装**（engines.yamlに定義あるがパーサーなし）

#### 16.13.3 除外
- 公開SearXインスタンス（searx.be等）→ サーバーサイド問題のため除外
- `yandex`: 将来拡張として保留（優先度低）
- `metager`: ログインゲート検出のため削除
...
- [x] E2Eスクリプト共通修正（captcha_detected→_is_captcha_error等）
- [x] MetaGer完全削除
- [x] Ecosia/Startpageセレクター修正（実HTMLに基づく）

#### 16.13.6 残作業 🔄

##### 優先度高（仕様書§3.2準拠に必要）
- [ ] **Bingパーサー実装**（engines.yamlに定義済み、パーサー未実装）
  ...
  - `tests/fixtures/search_html/bing_results.html` 作成
...
**成果物**:
- `config/engines.yaml`（7エンジン定義）
- `config/search_parsers.yaml`（7エンジンセレクター）
- `src/search/search_parsers.py`（7パーサー実装）
- `tests/test_search_parsers.py`（53テスト）
- `tests/scripts/verify_*_search.py`（3スクリプト成功）
- `tests/scripts/verify_*.py`（その他5スクリプト）
